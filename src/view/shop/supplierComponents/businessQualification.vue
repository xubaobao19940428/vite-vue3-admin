					...
					<el-col :span="8">
						<el-form-item label="合作合同" required>
							<c-upload
								:limit="6"
								ref="cooperationImg"
								:fileList="qualification.contract"
		...
		const startTimeValitor = (rule, value, callback) => {
			if (value != '' && value != null) {
				if (
					...
					this.qualification.endTimeStamp != null
				) {
					if (value >= this.qualification.endTimeStamp) {
						callback(
							new Error('合同开始时间不得大于等于合同截止时间')
						)
					} else {
						callback()
					}
		...
		const endTimeValitor = (rule, value, callback) => {
			if (value != '' && value != null) {
				if (
					...
					this.qualification.startTimeStamp != null
				) {
					if (value <= this.qualification.startTimeStamp) {
						callback(
							new Error('合同截止时间不得小于等于合同开始时间')
						)
					} else {
						callback()
					}
