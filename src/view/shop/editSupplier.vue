...
<template>
  <div class="edit_supplier">
    ...
    <!--资质&合同-->
    <business-qualification ref="shopQualification"></business-qualification>
  </div>
</template>

<script>
import supplierBaseInfo from './supplierComponents/supplierBaseInfo'
import businessScope from './supplierComponents/businessScope'
// import settlementType from './supplierComponents/settlementType';
import businessQualification from './supplierComponents/businessQualification'
import { addSeller, getSeller, editorSeller } from '@/api/newJson/shop'
export default {
  data() {
    return {
      id: '',
      ...
      getSeller({
        ...
        .then((response) => {
          if (response) {
            let _this = this
            _this.$nextTick(() => {
              _this.$refs['shopBaseInfo'].supplierDetail.contactPhone =
                response.data.contactPhone
              _this.$refs['shopBaseInfo'].supplierDetail.contactName =
                response.data.contactName
              _this.$refs['shopBaseInfo'].supplierDetail.contactEmail =
                response.data.contactEmail
              _this.$refs['shopBaseInfo'].supplierDetail.name =
                response.data.name
              _this.$refs['shopBaseInfo'].supplierDetail.shortName =
                response.data.shortName
              _this.$refs['shopBaseInfo'].supplierDetail.cooperationStatus =
                response.data.cooperationStatus
              if (response.data.remark) {
                _this.$refs['shopBusiness'].supplierScope.remark =
                  response.data.remark
              }

              _this.$refs['shopQualification'].qualification.endTimeStamp =
                response.data.endTimeStamp
              _this.$refs['shopQualification'].qualification.startTimeStamp =
                response.data.startTimeStamp
              _this.$refs['shopQualification'].qualification.contract =
                response.data.contract
              _this.$refs['shopQualification'].qualification.logo =
                response.data.logo
              _this.$refs['shopQualification'].qualification.enterpriseWeChat =
              ...
              _this.$refs['shopQualification'].$refs['logo'].setFileList(
                _this.$refs['shopQualification'].qualification.logo,
              )
        ...
        .then((data) => {
          if (data) {
            let shopInfo = {
              ...
              remark: this.$refs['shopBusiness'].supplierScope.remark,
            }
            if (this.id) {
              shopInfo.id = this.id
