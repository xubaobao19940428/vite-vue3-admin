	...
	<div class="app-container">
		<el-form inline>
			<el-form-item label="国家：">
				<el-select
					v-model="searchParam.countryCode"
					filterable
					clearable
				>
					<el-option
			...
			<el-form-item label="应用平台：">
				<el-select v-model="searchParam.appType">
					<el-option
						v-for="key in Object.keys(APP_TYPE)"
						:key="key"
						:label="APP_TYPE[key]"
						:value="parseInt(key)"
					></el-option>
				...
				>
					<template slot-scope="scope">
						...
						<el-button
							...
							v-if="
								hasPermission(
									'BTN_PRODUCT_LIST_AD_BATCH_DELETE'
								)
							"
...
export default {
	...
	methods: {
		...
		getList(page) {
			...
			listQualityRecommend(this.filterData(this.searchParam)).then(
				(res) => {
		...
		showEditDialog(type, data) {
			...
			if (type == 'edit') {
				...
				this.dialog.param.productId = this.dialog.param.productId
					? this.dialog.param.productId.join(',')
					: ''
			} else if (type === 'copy') {
				Object.assign(
					this.dialog.param,
