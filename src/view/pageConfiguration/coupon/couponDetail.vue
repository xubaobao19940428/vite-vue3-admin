<template>
    <div class="app-container">
        ...
        <el-form class="mar-t-20 " inline>
            ...
            <el-form-item>
                <el-button @click="getQrCode" type="primary" size="small">生成二维码</el-button>
            </el-form-item>
            <el-form-item label="优惠口令：">
                <el-input v-model="password" placeholder="输入数字、字母，区分大小写" style="width: 200px" size="medium"></el-input>
                <el-button @click="handleAddCouponPassword" type="primary" class="mar-l-10" size="small">提交</el-button>
            </el-form-item>
        </el-form>

        <!-- 以特定形式优惠券详情内容展示 -->
        <div v-if="Object.keys(couponDetail).length > 0" class="detail-wrapper">
            <el-form class="mar-t-20 " label-width="110px" label-position="right" label-suffix="：" inline>
                ...
                <el-form-item label="优惠内容">
                    ...
                    <span v-else-if="couponDetail.couponType == 2">优惠{{ couponDetail.discount }}%</span>
            ...
            <!-- 图表展示 -->
            <v-chart v-if="polar1.ready" :autoresize="true" :options="polar1" />
        </div>

        <!-- 搜索条件区域 -->
        <el-form class="mar-t-20 " inline>
            <el-form-item label="使用状态：">
                <el-select v-model="searchParam.couponUseStatus" size="medium">
                    <el-option :value="9" label="全部"></el-option>
                    <el-option v-for="(val, key) in useStatus" :key="key" :label="val" :value="key"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="订单编号：">
                <el-input v-model="searchParam.orderNo" size="medium"></el-input>
            </el-form-item>
            <el-form-item label="用户ID：">
                <el-input v-model="searchParam.userId" size="medium"></el-input>
            </el-form-item>
            <el-form-item>
                <el-button @click="getList(1)" type="primary" class="mar-l-10" size="small">搜索</el-button>
            </el-form-item>
        </el-form>

        <!-- 优惠券使用情况表格展示区域 -->
        <el-table :data="dataList" height="100%" border v-loading="load" style="width: 100%; min-height: 200px">
            <el-table-column prop="couponUserDopb.couponCode" min-width="200px" align="center" label="优惠码"></el-table-column>
            <el-table-column prop="couponUserDopb.userId" min-width="100px" align="center" label="领取用户ID"></el-table-column>
            <el-table-column prop="couponUserDopb.receivingChannel" align="center" label="领取方式"></el-table-column>
            <el-table-column label="优惠券有效期" align="center" min-width="320px">
            ...
            <el-table-column min-width="180px" align="center" label="使用时间">
                <template slot-scope="scope">
            ...
            <el-table-column prop="couponUserDopb.orderId" min-width="180px" align="center" label="订单编号"></el-table-column>
        </el-table>
        ...
        <el-dialog title="二维码" :visible.sync="qrCodeShow" width="500px" @close="qrCodeShow = false">
            <div class="qr-img-box">
                <img :src="qrCodeImg" />
                <el-button type="primary" class="mar-b-10" size="small">
...
export default {
    ...
    methods: {
        ...
        getList(page) {
            ...
            queryUserCouponManage({ query: this.filterData(this.searchParam) })
                .then(res => {
                    if (res.ret.errcode === 1) {
