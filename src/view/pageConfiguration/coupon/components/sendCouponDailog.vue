<template>
	...
	>
                    ...
                    <el-form-item label="导表上传：">
                        ...
                        >
                            ...
                            <div slot="tip" class="el-upload__tip tip">
                                通过上传表格，可批量完成优惠券发放，且系统为整体发放，如有一张发放失败则整体发放失败，无需担心发重发漏；因此请
                                <a download="文件" href="https://file.fingo.shop/fingo/templates/VoucherImportTemplate.xlsx" class="link">下载模板</a>
                                并按照模板中的指导说明制作表格。
                            </div>
                        </el-upload>
                    </el-form-item>
		...
		<span slot="footer" class="dialog-footer">
			<el-button @click="handleCloses" size="small">取 消</el-button>
			<el-button
				type="primary"
...
import { addOldUserCoupon } from '@/api/newJson/coupon.js'
	...
	methods: {
		...
		uploadFile(req) {
			uploadFileRequest(
				...
				(url) => {
					req.onSuccess(url, req.file, [req.file])
				},
				(err) => {
					req.onError(err)
				}
				...
				this.$refs['sendCouponForm'].validate((valid) => {
					if (valid) {
						...
						let params = {
							...
							couponId: Number(this.param.couponId),
						}

						addOldUserCoupon(this.filterData(params))
							.then((res) => {
								this.saving = false
								if (res) {
